...
import "./DeleteReqSale.css";

function DeleteReqSale() {
  const [search, setSearch] = useState("");
  const [pageSize, setPageSize] = useState("10");
  const [rows] = useState([]); // Table stays empty

  const columns = [
    "SL", "Date", "Type", "Invoice No", "Customer Name", "User",
    "Total Quantity", "Return Qty", "TAX", "Total Amount",
    "Paid", "Credit", "Status", "Action"
  ];
  ...
  return (
    <div className="deletereqsale-container">
      <div className="main-title">Delete Requested Sale</div>
      ...
      <div className="table-scroll-area">
        <table className="deletereqsale-table">
          <thead>
            <tr>
              {columns.map(col => (
                <th key={col}>{col}</th>
          ...
          <tbody>
            <tr>
              <td colSpan={columns.length} className="empty">
                {/* Table is intentionally empty as in your screenshot */}
              </td>
            </tr>
          </tbody>
        </table>
